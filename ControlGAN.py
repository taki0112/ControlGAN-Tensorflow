...
class ControlGAN():
    ...
    def d_train_step(self, real_256, caption, class_id):
        with tf.GradientTape() as d_64_tape, tf.GradientTape() as d_128_tape, tf.GradientTape() as d_256_tape :
            ...
            region_features_64, _ = self.cnn_encoder(real_64, training=True)
            region_features_128, _ = self.cnn_encoder(real_128, training=True)
            region_features_256, _ = self.cnn_encoder(real_256, training=True)
            region_features = [region_features_64, region_features_128, region_features_256]
            ...
            for i in range(3):
                ...
                each_d_word_loss = self.word_weight * word_level_correlation_loss(region_features[i], word_emb, class_id, gamma1=4.0, gamma2=5.0)

                d_adv_loss.append(each_d_adv_loss)
                d_word_loss.append(each_d_word_loss)

                d_each_loss.append(each_d_adv_loss + each_d_word_loss)

            d_loss = tf.reduce_sum(d_each_loss)
        ...
        for i in range(3):
            d_gradient = d_tape[i].gradient(d_each_loss[i], d_train_variable[i])
            self.d_optimizer[i].apply_gradients(zip(d_gradient, d_train_variable[i]))

        return d_loss, tf.reduce_sum(d_adv_loss), tf.reduce_sum(d_word_loss)

    def g_train_step(self, real_256, caption, class_id):
        with tf.GradientTape() as g_tape:
            ...
            real_64, real_128 = resize(real_256, target_size=[64, 64]), resize(real_256, target_size=[128, 128])
            fake_64, fake_128, fake_256 = fake_imgs
            real_imgs = [real_64, real_128, real_256]
            ...
            g_loss = g_adv_loss + g_vgg_loss + g_kl_loss + g_embed_loss
        ...
        return g_loss, g_adv_loss, g_vgg_loss, g_kl_loss, g_embed_loss, fake_256
    ...
    def train(self):
        ...
        for idx in range(self.start_iteration, self.iteration):
            ...
            d_loss, d_adv_loss, d_word_loss = self.d_train_step(real_256, caption, class_id)

            g_loss, g_adv_loss, g_vgg_loss, g_kl_loss, g_embed_loss, fake_256 = self.g_train_step(real_256, caption, class_id)
            g_loss += embed_loss

            # save to tensorboard
            with train_summary_writer.as_default():
                tf.summary.scalar('g_adv_loss', g_adv_loss, step=idx)
                tf.summary.scalar('g_vgg_loss', g_vgg_loss, step=idx)
                ...
                tf.summary.scalar('d_word_loss', d_word_loss, step=idx)
    ...
    def model_dir(self):
        ...
        return "{}_{}_{}_{}adv_{}kl_{}vgg_{}word_{}embed{}".format(self.model_name, self.dataset_name, self.gan_type,
                                                           self.adv_weight, self.kl_weight, self.vgg_weight, self.word_weight, self.embed_weight,
                                                           sn)
