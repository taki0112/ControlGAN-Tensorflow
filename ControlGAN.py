...
class_vgg_16 = Vgg16_class()
class_inception_v3 = Inception_v3_class()

class ControlGAN():
    def __init__(self, sess, args):

        self.phase = args.phase
        self.model_name = 'ControlGAN'
        ...
        self.word_weight = args.word_weight
        ...
        print("# word_weight : ", self.word_weight)
    ...
    def generator(self, z_code, sent_emb, word_emb, mask, is_training=True, reuse=tf.AUTO_REUSE, scope='generator'):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            c_code, att_1 = spatial_attn_net(h_code1, word_emb, mask, channels, use_bias=False, sn=self.sn, scope='128_spa_attention_net')
            c_code_channel, att_channel = channel_attn_net(c_code, word_emb, use_bias=False, sn=self.sn, scope='128_ch_attention_net')

            c_code = tf.reshape(c_code, shape=[h_code1.shape[0], h_code1.shape[1], h_code1.shape[2], -1])
            h_c_code = tf.concat([h_code1, c_code], axis=-1)
            h_c_c_code = tf.concat([h_c_code, c_code_channel], axis=-1)

            for i in range(2):
                h_c_c_code = resblock(h_c_c_code, channels * 3, is_training, use_bias=False, sn=self.sn, scope='128_resblock_' + str(i))

            h_code2 = up_block(h_c_c_code, channels, is_training, use_bias=False, sn=self.sn, scope='128_up_block_' + str(i))
            ...
            # 256 img
            c_code, att_2 = spatial_attn_net(h_code2, word_emb, mask, channels, use_bias=False, sn=self.sn, scope='256_spa_attention_net')
            c_code_channel, att_channel = channel_attn_net(c_code, word_emb, use_bias=False, sn=self.sn, scope='256_ch_attention_net')

            c_code = tf.reshape(c_code, shape=[h_code2.shape[0], h_code2.shape[1], h_code2.shape[2], -1])
            h_c_code = tf.concat([h_code2, c_code], axis=-1)
            h_c_c_code = tf.concat([h_c_code, c_code_channel], axis=-1)

            for i in range(2):
                h_c_c_code = resblock(h_c_c_code, channels * 3, is_training, use_bias=False, sn=self.sn, scope='256_resblock_' + str(i))

            h_code3 = up_block(h_c_c_code, channels, is_training, use_bias=False, sn=self.sn, scope='256_up_block_' + str(i))
    ...
    def discriminator(self, x_init, sent_emb, is_training=True, reuse=tf.AUTO_REUSE, scope='discriminator'):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            emb_feature = get_inception_feature(x_64, channel=self.embed_dim, class_inception_v3=class_inception_v3, sn=self.sn)
            emb_features.append(emb_feature)

            emb_feature = get_inception_feature(x_128, channel=self.embed_dim, class_inception_v3=class_inception_v3, sn=self.sn)
            emb_features.append(emb_feature)

            emb_feature = get_inception_feature(x_256, channel=self.embed_dim, class_inception_v3=class_inception_v3, sn=self.sn)
            emb_features.append(emb_feature)
        ...
        return uncond_logits, cond_logits, emb_features
    ...
    def model_dir(self):
        ...
        return "{}_{}_{}_{}adv_{}kl_{}vgg_{}word{}".format(self.model_name, self.dataset_name, self.gan_type,
                                                           self.adv_weight, self.kl_weight, self.vgg_weight, self.word_weight,
                                                           sn)
