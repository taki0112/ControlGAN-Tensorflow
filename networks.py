...
class ChannelAttention(tf.keras.layers.Layer):
    def __init__(self, channels, name='ChannelAttention'):
        super(ChannelAttention, self).__init__(name=name)

        self.channels = channels # channels = h * w
        self.word_conv = Conv(self.channels, kernel=1, stride=1, use_bias=False, name='word_conv')

    def build(self, input_shape):
        self.bs, self.h, self.w, self.context_ch = input_shape[0]
        self.seq_len = input_shape[1][1] # length of source

    def call(self, inputs, training=True):
        weighted_context, word_emb = inputs # context = word_emb
        ...
        weighted_context = tf.reshape(weighted_context, [self.bs, self.h * self.w, -1])
        attn_c = tf.matmul(weighted_context, word_emb, transpose_a=True, transpose_b=True)  # [bs, idf, seq_len]
        attn_c = tf.reshape(attn_c, [self.bs * self.context_ch, self.seq_len])
        attn_c = tf.nn.softmax(attn_c)

        attn_c = tf.reshape(attn_c, [self.bs, self.context_ch, self.seq_len])
        weightedContext_c = tf.matmul(word_emb, attn_c, transpose_a=True, transpose_b=True)
        weightedContext_c = tf.reshape(weightedContext_c, [self.bs, self.h, self.w, self.context_ch])
...
class Generator_128(tf.keras.layers.Layer):
    def __init__(self, channels, name='Generator_128'):
        ...
        self.channel_attention = ChannelAttention(channels=64 * 64)
    ...
    def call(self, inputs, training=True):
        ...
        c_code_ch_attention, _ = self.channel_attention([c_code, word_emb])

        h_c_code = tf.concat([h_code, c_code, c_code_ch_attention], axis=-1)
...
class Generator_256(tf.keras.layers.Layer):
    def __init__(self, channels, name='Generator_256'):
        ...
        self.spatial_attention = SpatialAttention(channels=self.channels)
        self.channel_attention = ChannelAttention(channels=128 * 128)
        self.model = self.architecture()
...
class Vgg16(tf.keras.Model):
    def __init__(self, name='Vgg16'):
        ...
        self.model = tf.keras.Model(inputs=self.vgg16.input, outputs=self.vgg16.get_layer('block2_conv2').output)
        # block4_conv3 -> MUNIT
        # block2_conv2
