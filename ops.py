...
def word_level_correlation_loss(img_feature, word_emb, class_id, gamma1=4.0, gamma2=5.0):
    ...
    seq_len = word_emb.shape[1] # words_num
    ...
    for i in range(batch_size):
        ...
        weiContext, _ = func_attention(context, word) # [1, embed_dim=256, seq_len]
        # weiContext = tf.transpose(weiContext, perm=[0, 2, 1]) # [1, seq_len, embed_dim=256]

        aver = tf.reduce_mean(word, axis=1, keepdims=True)
        res_word = tf.matmul(aver, word, transpose_b=True) # [1, 1, seq_len]
        res_softmax = tf.nn.softmax(res_word, axis=-1)
        res_softmax = tf.tile(res_softmax, multiples=[1, weiContext.shape[1], 1])
        self_weiContext = weiContext * res_softmax

        word = tf.reshape(word, shape=[seq_len, -1])
        self_weiContext = tf.transpose(self_weiContext, perm=[0, 2, 1]) # [1, seq_len, embed_dim]
        self_weiContext = tf.reshape(self_weiContext, shape=[seq_len, -1])

        row_sim = cosine_similarity(word, self_weiContext)
        row_sim = tf.reshape(row_sim, shape=[1, seq_len])

        row_sim = tf.exp(row_sim * gamma2)
        row_sim = tf.reduce_sum(row_sim, axis=-1, keepdims=True)
        row_sim = tf.math.log(row_sim)

        similar_list.append(row_sim[0][0])
